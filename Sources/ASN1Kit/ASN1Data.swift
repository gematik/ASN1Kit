...
public enum ASN1Data {
    ...
    public var length: Int {
        ...
        case .constructed(let items):
            return items.reduce(0) { acc, item in
                return acc + item.length + item.length.lengthSize + item.tag.length
            }
        }
    }
    ...
    public var items: [ASN1Object]? {
        ...
        if case .constructed(let items) = self {
            return items
        }
        return nil
    }
    ...
    public var primitive: Data? {
        if case .primitive(let data) = self {
            return data
        }
        return nil
    }
...
extension ASN1Data {
    ...
    public func fold<T>(_ primitive: @escaping (Data) throws -> T,
                        _ constructed: ([ASN1Object]) throws -> T) rethrows -> T {
        switch self {
        case .primitive(let data):
